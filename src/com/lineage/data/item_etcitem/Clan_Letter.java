...
public class Clan_Letter extends ItemExecutor {
		...
		if (target != null) {
			...
			if (CharacterTable.doesCharNameExist(name)) {
				try {
					...
					final CopyOnWriteArrayList<L1ItemInstance> list = CharItemsReading.get().loadItems(objid);
	...
	private void saveLetter(final int itemObjectId, final int code, final String sender,
		...
		LetterTable.getInstance().writeLetter(itemObjectId, code, sender, receiver, date, 0, subject,
				content);
	}
}
