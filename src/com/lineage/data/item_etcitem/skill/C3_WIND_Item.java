...
/**
 * C3_WIND [風傷術] 每次使用60秒 只對自身周圍玩家有效 使用風傷術之後，不會增加武器上的風系傷害， 但是他會對施法者周圍的玩家造成風系傷害，當
 * 角色被敵人包圍時正是使用風傷術的大好機會 但敵方抗性越高受到的傷害越小 等級50-70 消耗三國貨幣X1 15-30傷害 等級70-90 消耗三國貨幣X1
	...
	public void execute(final int[] data, final L1PcInstance pc, final L1ItemInstance item) {
		try {
			...
			if (pc.hasSkillEffect(L1SkillId.C3_WIND)) {// 風傷術
				pc.sendPackets(
						new S_ServerMessage("\\fV[風傷術]效果尚存:" + pc.getSkillEffectTimeSec(L1SkillId.C3_WIND)));
				return;
			}
			...
			pc.setSkillEffect(L1SkillId.C3_WIND, _time * 1000);
