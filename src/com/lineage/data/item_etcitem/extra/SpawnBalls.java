...
/**
 * 仿正道具：靈魂球_死亡騎士 (怪物編號/數量/地圖X/地圖Y/地圖編號/召喚是否公告)
 * 
 * @author Smile
 */
public class SpawnBalls extends ItemExecutor {
	...
	public void execute(final int[] data, final L1PcInstance pc, final L1ItemInstance item) {
		...
		for (L1Object object : World.get().getVisibleObjects(_mapid).values()) {
			if (object instanceof L1MonsterInstance) {
				...
				if (mon.getNpcId() == _mobId) {
					pc.sendPackets(new S_SystemMessage("\\aG該怪物已經存在囉!"));
					return;
				}
			}
		}
		...
		if (_isShow) {
			World.get().broadcastChatAll(new S_BoxMessage("\\aD有人召喚出了強大的" + npcTemp.get_name()));
		}
	}
