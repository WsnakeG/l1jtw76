...
public class ItemSpiritCrystal extends ItemExecutor {
    ...
    public void execute(int[] data, L1PcInstance pc, L1ItemInstance item) {
        ...
        String nameId = parts[1]; // 例：$4717
        int deleteFlag = 1; // 預設刪除
        if (parts.length >= 3) {
            try {
        ...
        if (skillId == -1) {
            pc.sendPackets(new S_ServerMessage("找不到技能")); // 找不到技能
            return;
        }
        ...
        case 174: // 精準射擊
            if (data == null || data.length < 3 || data[0] == 0) {
                pc.sendPackets(new S_ServerMessage("請點選目標施放此技能。"));
                return;
            }
            targetId = data[0];
            targetX = data[1];
        ...
        // 顯示成功訊息（可自訂）
        pc.sendPackets(new S_ServerMessage("手中的精靈水晶散發出耀眼的光芒")); // 魔法施放成功
    }
