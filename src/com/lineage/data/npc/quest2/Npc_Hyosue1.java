 ...
 * @author terry0412
 */
public class Npc_Hyosue1 extends NpcExecutor {

	private static final Log _log = LogFactory.getLog(Npc_Hyosue1.class);
	...
	public void talk(final L1PcInstance pc, final L1NpcInstance npc) {
		try {
			if (pc.getLawful() < -1000) {
				pc.sendPackets(new S_NPCTalkReturn(npc.getId(), "hyosue5"));
				return;
			}
			...
			if (day_of_week == 1 // 星期日
				...
				// 只能透過沙塵暴龍捲風隨機傳送入場
				pc.sendPackets(new S_NPCTalkReturn(npc.getId(), "Hyosue"));
			}
	...
	public void action(final L1PcInstance pc, final L1NpcInstance npc, final String cmd, final long amount) {
		if (cmd.equalsIgnoreCase("a")) { // 觀察沙蟲領域
			...
			// 進入觀察模式 - 持續 300秒
			pc.beginGhost(32699, 32896, (short) 96, true, 300);
			...
			if (day_of_week == 1 // 星期日
				...
				if (!pc.getInventory().consumeItem(L1ItemId.ADENA,
						(long) (10000 * ConfigRate.RATE_DROP_ADENA))) {
					// 金幣不夠
					pc.sendPackets(new S_NPCTalkReturn(npc.getId(), "hyosue1"));
					return;
				}
				...
				pc.setTeleportY(32896);
				pc.setTeleportMapId((short) 96);
				// 送出鎖定封包
