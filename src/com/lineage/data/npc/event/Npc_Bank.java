			...
			if (cmd.equalsIgnoreCase("A")) {// 進入銀行作業
				...
				if (!bankType._type && (bank != null)) {
					if (bank.get_adena_count() >= BankSet.BANKMAX) {
						...
						if (item != null) {
							final long count = amount + bank.get_adena_count();
							...
							pc.sendPackets(new S_ServerMessage("\\fU你要求的 " + item.getName() + "("
									+ RangeLong.scount(amount) + ") 已經存入銀行"));
							...
							pc.sendPackets(
									new S_ServerMessage("\\fR你身上的 " + itemtmp.getNameId() + " 數量低於您要求存入的數字"));
						}
					}
				}
				...
				if (bankType._type && (bank != null)) {
					...
					final long newCount = bank.get_adena_count() - count;
				...
				if (!pass.equals("n")) {
					...
					if (bankType._pass_x1.length() == 6) {
						...
						case 0:
							...
							out = "*";
							pc.sendPackets(
									new S_NPCTalkReturn(npc.getId(), "1y_bank_5", new String[] { out }));
							break;
							...
							if (bankType._pass.length() == 6) {
								if (bankType._pass_x1.equals(bankType._pass)) {
									if (!bankType._repass) {
										...
										bank.set_pass(bankType._pass);
										// 建立帳戶資料
										AccountBankReading.get().updatePass(bankType._account_name,
												bankType._pass);
						...
						case 0:
							...
							out = "*";
							pc.sendPackets(
									new S_NPCTalkReturn(npc.getId(), "1y_bank_4", new String[] { out }));
							break;
						...
						case 5:
							...
							if (bankType._pass_x1.length() == 6) {
								pc.sendPackets(
										new S_NPCTalkReturn(npc.getId(), "1y_bank_5", new String[] { "" }));
						...
						if (bankType._pass.length() == 6) {
							if (bankType._pass.equals(bank.get_pass())) {
								if (bankType._repass) {
									...
									pc.sendPackets(
											new S_ItemCount(npc.getId(), 100, 2000000000, "1y_bank_3", "O"));
								}
							...
							pc.sendPackets(
									new S_NPCTalkReturn(npc.getId(), "1y_bank_8", new String[] { out }));
						}
						break;
					}
				}
