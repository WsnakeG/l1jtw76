...
public class NpcChatTable {
	...
	public void load() {
		...
		_log.info("載入NPC會話資料數量: " + (_npcChatAppearance.size() + _npcChatDead.size() + _npcChatHide.size()
				+ _npcChatGameTime.size()) + "(" + timer.get() + "ms)");
	}
	...
	public Collection<L1NpcChat> all() {
		try {
			...
			return (vs != null) ? vs
					: (_allTimeValues = Collections.unmodifiableCollection(_npcChatGameTime.values()));
