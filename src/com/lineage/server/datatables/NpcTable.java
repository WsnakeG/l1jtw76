...
public class NpcTable {
	...
	private NpcExecutor addClass(final int npcid, final String className) {
		try {
			...
			final String error = "發生[NPC檔案]錯誤, 檢查檔案是否存在:" + className + " NpcId:" + npcid;
			_log.error(error);
	...
	public L1NpcInstance newNpcInstance(final int id) {
		try {
			...
			_log.error("newNpcInstance1 NPCID:" + id + "/" + e.getLocalizedMessage(), e);
		}
		return null;
	}
	...
	public L1NpcInstance newNpcInstance(final L1Npc template) {
		try {
			...
			return (L1NpcInstance) con.newInstance(template);

		} catch (final Exception e) {
			_log.error("newNpcInstance2 NPCID:" + template.get_npcId() + "/" + e.getLocalizedMessage(), e);
		}
		return null;
	}
