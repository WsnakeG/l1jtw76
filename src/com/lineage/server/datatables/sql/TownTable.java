...
public class TownTable implements TownStorage {
	...
	public synchronized void addSalesMoney(final int town_id, final int salesMoney) {
		...
		try {
			...
			pstm = con.prepareStatement(
					"UPDATE `server_town` SET " + "`sales_money`=`sales_money`+?,`town_tax`=`town_tax`+?,"
	...
	public void updateSalesMoneyYesterday() {
		...
		try {
			con = DatabaseFactory.get().getConnection();
			pstm = con.prepareStatement(
					"UPDATE `server_town` SET " + "`sales_money_yesterday`=`sales_money`,`sales_money`=0");
			pstm.execute();
	...
	public String totalContribution(final int townId) {
		...
		try {
			...
			pstm5 = con.prepareStatement(
					"UPDATE `server_town` SET " + "`leader_id`=?,`leader_name`=?,`tax_rate`= 0,"
