	...
	public void load() {
		...
		try {
			...
			while (rs.next()) {
				...
				final Calendar deadline = timestampToCalendar((Timestamp) rs.getObject("deadline"));
	...
	public void insertAuctionBoard(final L1AuctionBoardTmp board) {
		...
		try {
			...
			pstm = con.prepareStatement(
					"INSERT INTO `server_board_auction` SET " + "`house_id`=?,`house_name`=?,`house_area`=?,"
	...
	public void updateAuctionBoard(final L1AuctionBoardTmp board) {
		...
		try {
			...
			pstm = con.prepareStatement("UPDATE `server_board_auction` SET "
					+ "`house_name`=?,`house_area`=?,`deadline`=?," + "`price`=?,`location`=?,`old_owner`=?,"
