...
public class L1WarSpawn {
	 ...
	 * final int locy, final int mapid, final int h) { try { final L1Npc l1npc =
	 * NpcTable.get().getTemplate(npcid); if (l1npc != null) { final
	 * L1NpcInstance npc = NpcTable.get().newNpcInstance(l1npc);
	 * npc.setId(IdFactoryNpc.get().nextId()); npc.setX(locx); npc.setY(locy);
	 * npc.setHomeX(locx); npc.setHomeY(locy); npc.setHeading(h);
	 * npc.setMap((short) mapid); World.get().storeObject(npc);
	 * World.get().addVisibleObject(npc); for (final L1PcInstance pc :
	 * World.get().getAllPlayers()) { npc.addKnownObject(pc);
	 * pc.addKnownObject(npc); pc.sendPacketsAll(new S_NPCPack(npc)); } return
	 * npc; } } catch (final Exception e) { _log.error(e.getLocalizedMessage(),
	 * e); } return null; }
	 */
}
