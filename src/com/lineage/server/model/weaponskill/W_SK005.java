...
/**
 * 武器攻擊附加MP奪取(附加傷害) 使用這項技能武器將會在攻擊時隨設定機率附加MP奪取(以原始輸出傷害計算) _type1:最小吸收質
		...
		try {
			...
			if (random >= chance) {
				if (target.getCurrentMp() > 0) {
					final int evel = weapon.getEnchantLevel() * 2;
					int mpadd = (int) dmg2(srcdmg) + evel;
					...
					final short newMp = (short) (pc.getCurrentMp() + mpadd);
