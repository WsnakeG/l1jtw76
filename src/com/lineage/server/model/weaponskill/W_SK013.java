...
/**
 * 特殊系列武器傷害公式-13【體質隨機數】*2+ 【力量+敏捷+智慧】*2 +【浮動0-50】
 * 發動機率：基礎機率3% 隨武器加成提高增加 +2=5% +4=7% +5=8% +7=10% 之後每+2 多1% 
 * _type1:體質隨機倍率 
 * _type2:力量+敏捷+智慧隨機值倍率 
 * _type3:傷害倍率 計算後數字加入傷害
 * 
 * @author erics4179
 */
public class W_SK013 extends L1WeaponSkillType {

	private static final Log _log = LogFactory.getLog(W_SK013.class);
	...
	public double start_weapon_skill(final L1PcInstance pc, final L1Character target,
		...
		try {
			...
			if (random >= chance) {
				// 【體質隨機數】*2+ 【力量+敏捷+智慧】*2 +【浮動0-50】
				// 【type1】*2+ 【type2】*2 +【type3】
				double damage = ((_random.nextInt(pc.getCon()) + 1) * _type1)
						+ ((_random.nextInt(pc.getStr() + pc.getDex() + pc.getInt()) + 1) * _type2) + +dmg1();
