...
public class L1CN implements L1CommandExecutor {
	...
	public void execute(final L1PcInstance pc, final String cmdName, final String arg) {
		try {
			...
			final L1Account acc = AccountReading.get().getAccount(accname.toLowerCase());
			if (acc != null) {
				...
				if (item != null) {
					...
					pc.sendPackets(new S_ServerMessage(166,
							"指定帳號(" + acc.get_login() + ") 加入物品" + item_id + " 數量" + count));
					DwarfReading.get().insertItem(acc.get_login(), item);

					final ClientExecutor cl = OnlineUser.get().get(acc.get_login());
					if (cl != null) {
						final L1PcInstance tgpc = cl.getActiveChar();
						if (tgpc != null) {
							tgpc.sendPackets(
									new S_ServerMessage(166, "GM在您的倉庫加入物品" + item.getName() + " 數量" + count));
							// 重整倉庫數據
