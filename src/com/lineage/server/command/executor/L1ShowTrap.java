...
public class L1ShowTrap implements L1CommandExecutor {
	...
	public void execute(final L1PcInstance pc, final String cmdName, final String arg) {
		 ...
		 * pc.setSkillEffect(GMSTATUS_SHOWTRAPS, 0); } else if
		 * (arg.equalsIgnoreCase("off")) {
		 * pc.removeSkillEffect(GMSTATUS_SHOWTRAPS); for (final L1Object obj :
		 * pc.getKnownObjects()) { if (obj instanceof L1TrapInstance) {
		 * pc.removeKnownObject(obj); pc.sendPackets(new S_RemoveObject(obj)); }
		 * } } else { // 261 \f1指令錯誤。 pc.sendPackets(new S_ServerMessage(261));
		 * }
		 */
	}
}
